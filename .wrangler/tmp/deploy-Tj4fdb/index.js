...
async function handleCallCompleted(payload, env) {
...
__name(handleCallCompleted, "handleCallCompleted");
async function handleCallTranscript(payload, env) {
  const transcriptData = payload.data.object;
  const callId = transcriptData.callId;
  const transcript = transcriptData.dialogue.map((d) => `${d.identifier}: ${d.content}`).join("\n");
  console.log(`Call transcript ready for call ${callId}:
${transcript}`);
}
__name(handleCallTranscript, "handleCallTranscript");
async function handleMessageReceived(payload, env) {
...
__name(handleMessageReceived, "handleMessageReceived");
async function findRecentThreadForCustomer(customerId, env) {
  const query = `
...
__name(findRecentThreadForCustomer, "findRecentThreadForCustomer");
async function handlePlainChatMessage(payload, env) {
...
__name(handlePlainChatMessage, "handlePlainChatMessage");
async function lookupHubSpotContact(phoneNumber, env) {
  try {
    const encodedPhone = encodeURIComponent(phoneNumber);
    const response = await fetch(`https://outbound-dashboard.mapleinc.workers.dev/api/hubspot/search/${encodedPhone}`);
    if (!response.ok) {
      console.log(`HubSpot lookup failed for ${phoneNumber}: ${response.status}`);
      return null;
    }
    const data = await response.json();
    if (data.found) {
      if (data.contacts && data.contacts.length > 0) {
        const contact = data.contacts[0].properties;
      ...
      if (data.companies && data.companies.length > 0) {
        const company = data.companies[0].properties;
...
__name(lookupHubSpotContact, "lookupHubSpotContact");
async function upsertCustomerInPlain(phoneNumber, hubspotContact, env) {
  const mutation = `
  ...
  `;
  const fullName = hubspotContact ? `${hubspotContact.firstName} ${hubspotContact.lastName}`.trim() || phoneNumber : phoneNumber;
  const email = hubspotContact?.email ? { email: hubspotContact.email, isVerified: false } : { email: `${phoneNumber.replace("+", "")}@phone.maple.inc`, isVerified: false };
  const variables = {
    input: {
      identifier: {
...
__name(upsertCustomerInPlain, "upsertCustomerInPlain");
async function createThreadInPlain(customerId, title, env) {
  const mutation = `
...
__name(createThreadInPlain, "createThreadInPlain");
async function createThreadEvent(threadId, title, description, env) {
  const mutation = `
...
__name(createThreadEvent, "createThreadEvent");
async function sendChatToPlain(customerId, threadId, message, media, env) {
  const mutation = `
...
__name(sendChatToPlain, "sendChatToPlain");
async function sendSMSViaOpenPhone(to, message, env) {
...
__name(sendSMSViaOpenPhone, "sendSMSViaOpenPhone");
